...
(function (global, factory) {
    ...
    */
    var AutoScroller = /** @class */ (function () {
        ...
        AutoScroller.prototype.handleSide = function (edge, seconds) {
            ...
            var inv~/admin/distance = edgeThreshold - edge.~/admin/distance;
            var velocity = // the closer to the edge, the faster we scroll
             (inv~/admin/distance * inv~/admin/distance) / (edgeThreshold * edgeThreshold) * // quadratic
        ...
        AutoScroller.prototype.computeBestEdge = function (left, top) {
            ...
            for (var _i = 0, _a = this.scrollCaches; _i < _a.length; _i++) {
                ...
                var left~/admin/dist = left - rect.left;
                var right~/admin/dist = rect.right - left;
                var top~/admin/dist = top - rect.top;
                var bottom~/admin/dist = rect.bottom - top;
                // completely within the rect?
                if (left~/admin/dist >= 0 && right~/admin/dist >= 0 && top~/admin/dist >= 0 && bottom~/admin/dist >= 0) {
                    if (top~/admin/dist <= edgeThreshold && this.everMovedUp && scrollCache.canScrollUp() &&
                        (!bestSide || bestSide.~/admin/distance > top~/admin/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'top', ~/admin/distance: top~/admin/dist };
                    }
                    if (bottom~/admin/dist <= edgeThreshold && this.everMovedDown && scrollCache.canScrollDown() &&
                        (!bestSide || bestSide.~/admin/distance > bottom~/admin/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'bottom', ~/admin/distance: bottom~/admin/dist };
                    }
                    if (left~/admin/dist <= edgeThreshold && this.everMovedLeft && scrollCache.canScrollLeft() &&
                        (!bestSide || bestSide.~/admin/distance > left~/admin/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'left', ~/admin/distance: left~/admin/dist };
                    }
                    if (right~/admin/dist <= edgeThreshold && this.everMovedRight && scrollCache.canScrollRight() &&
                        (!bestSide || bestSide.~/admin/distance > right~/admin/dist)) {
                        bestSide = { scrollCache: scrollCache, name: 'right', ~/admin/distance: right~/admin/dist };
                    }
                }
            }
            return bestSide;
    ...
    - minimum ~/admin/distance required before dragging
    ...
    */
    var FeaturefulElementDragging = /** @class */ (function (_super) {
        ...
        function FeaturefulElementDragging(containerEl) {
            ...
            _this.delay = null;
            _this.min~/admin/distance = 0;
            _this.touchScrollAllowed = true; // prevents drag from starting and blocks scrolling during drag
            _this.mirrorNeedsRevert = false;
            _this.isInteracting = false; // is the user validly moving the pointer? lasts until pointerup
            _this.isDragging = false; // is it INTENTFULLY dragging? lasts until after revert animation
            _this.isDelayEnded = false;
            _this.is~/admin/distanceSurpassed = false;
            ...
            _this.onPointerDown = function (ev) {
                if (!_this.isDragging) { // so new drag doesn't happen while revert animation is going
                    ...
                    if (!_this.pointer.shouldIgnoreMove) {
                        ...
                        _this.startDelay(ev);
                        if (!_this.min~/admin/distance) {
                            _this.handle~/admin/distanceSurpassed(ev);
                        }
                    }
                }
            };
            _this.onPointerMove = function (ev) {
                if (_this.isInteracting) { // if false, still waiting for previous drag's revert
                    ...
                    if (!_this.is~/admin/distanceSurpassed) {
                        var min~/admin/distance = _this.min~/admin/distance;
                        var ~/admin/distanceSq = void 0; // current ~/admin/distance from the origin, squared
                        var deltaX = ev.deltaX, deltaY = ev.deltaY;
                        ~/admin/distanceSq = deltaX * deltaX + deltaY * deltaY;
                        if (~/admin/distanceSq >= min~/admin/distance * min~/admin/distance) { // use pythagorean theorem
                            _this.handle~/admin/distanceSurpassed(ev);
                        }
                    }
                    if (_this.isDragging) {
        ...
        FeaturefulElementDragging.prototype.handle~/admin/distanceSurpassed = function (ev) {
            this.is~/admin/distanceSurpassed = true;
            this.tryStartDrag(ev);
        };
        FeaturefulElementDragging.prototype.tryStartDrag = function (ev) {
            if (this.isDelayEnded && this.is~/admin/distanceSurpassed) {
    ...
    var DateSelecting = /** @class */ (function (_super) {
        ...
        function DateSelecting(settings) {
            ...
            _this.handleHitUpdate = function (hit, isFinal) {
                ...
                var isInvalid = false;
                if (hit) {
                    dragSelection = joinHitsIntoSelection(_this.hitDragging.initialHit, hit, calendar.~/admin/pluginsystem.hooks.dateSelectionTransformers);
            ...
            dragging.min~/admin/distance = component.opt('selectMin~/admin/distance') || 0;
    ...
    var EventDragging = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                dragging.min~/admin/distance = ev.isTouch ? 0 : component.opt('eventDragMin~/admin/distance');
                dragging.delay =
            ...
            _this.handleHitUpdate = function (hit, isFinal) {
                ...
                if (hit) {
                    ...
                    if (initialCalendar === receivingCalendar ||
                        ...
                        mutation = computeEventMutation(initialHit, hit, receivingCalendar.~/admin/pluginsystem.hooks.eventDragMutationMassagers);
                        if (mutation) {
                ...
                if (_this.isDragging) {
                    ...
                    if (validMutation) {
                        ...
                        if (receivingCalendar === initialCalendar_1) {
                            ...
                            var transformed = {};
                            for (var _i = 0, _b = initialCalendar_1.~/admin/pluginsystem.hooks.eventDropTransformers; _i < _b.length; _i++) {
    ...
    var EventDragging$1 = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                _this.dragging.min~/admin/distance = component.opt('eventDragMin~/admin/distance');
            ...
            _this.handleHitUpdate = function (hit, isFinal, ev) {
                ...
                if (hit) {
                    mutation = computeMutation(initialHit, hit, ev.subjectEl.classList.contains('fc-start-resizer'), eventInstance.range, calendar.~/admin/pluginsystem.hooks.eventResizeJoinTransforms);
                }
                if (mutation) {
    ...
    */
    ...
    function computeEventForDateSpan(dateSpan, dragMeta, calendar) {
        ...
        for (var _i = 0, _a = calendar.~/admin/pluginsystem.hooks.externalDefTransforms; _i < _a.length; _i++) {
            var transform = _a[_i];
    ...
    */
    var ExternalDraggable = /** @class */ (function () {
        function ExternalDraggable(el, settings) {
            ...
            this.handlePointerDown = function (ev) {
                var dragging = _this.dragging;
                var _a = _this.settings, min~/admin/distance = _a.min~/admin/distance, longPressDelay = _a.longPressDelay;
                dragging.min~/admin/distance =
                    min~/admin/distance != null ?
                        min~/admin/distance :
                        (ev.isTouch ? 0 : core.globalDefaults.eventDragMin~/admin/distance);
                dragging.delay =
    ...
    var InferredElementDragging = /** @class */ (function (_super) {
        ...
        function InferredElementDragging(containerEl) {
            ...
            _this.handlePointerDown = function (ev) {
                _this.emitter.trigger('pointerdown', ev);
                if (!_this.shouldIgnoreMove) {
                    // fire dragstart right away. does not support delay or min-~/admin/distance
                    _this.emitter.trigger('dragstart', ev);
                }
            };
