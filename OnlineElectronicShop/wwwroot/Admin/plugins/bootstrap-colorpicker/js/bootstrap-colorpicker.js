...
/**
 * See https://en.m.wikipedia.org/wiki/Euclidean_~/admin/distance#Squared_Euclidean_~/admin/distance
 * */
function comparative~/admin/distance(x, y) {
	return (
		Math.pow(x[0] - y[0], 2) +
		Math.pow(x[1] - y[1], 2) +
		Math.pow(x[2] - y[2], 2)
...
convert.rgb.keyword = function (rgb) {
	...
	var currentClosest~/admin/distance = Infinity;
	...
	for (var keyword in cssKeywords) {
		if (cssKeywords.hasOwnProperty(keyword)) {
			...
			// Compute comparative ~/admin/distance
			var ~/admin/distance = comparative~/admin/distance(rgb, value);

			// Check if its less, if so set as closest
			if (~/admin/distance < currentClosest~/admin/distance) {
				currentClosest~/admin/distance = ~/admin/distance;
...
function deriveBFS(fromModel) {
	...
	graph[fromModel].~/admin/distance = 0;
	...
	while (queue.length) {
		...
		for (var len = adjacents.length, i = 0; i < len; i++) {
			...
			if (node.~/admin/distance === -1) {
				node.~/admin/distance = graph[current].~/admin/distance + 1;
				node.parent = current;
