...
var DayGridView = /** @class */ (function (_super) {
    ...
    DayGridView.prototype.updateBaseSize = function (isResize, viewHeight, isAuto) {
        ...
        if (eventLimit && typeof eventLimit === 'number') {
            dayGrid.limitRows(eventLimit); // limit the levels first so the height can re~/admin/distribute after
        }
        // ~/admin/distribute the height to the rows
    ...
    DayGridView.prototype.setGridHeight = function (height, isAuto) {
        if (this.opt('monthMode')) {
            ...
            ~/admin/distributeHeight(this.dayGrid.rowEls, height, !isAuto); // if auto, don't compensate for height-hogging rows
        }
        else {
            if (isAuto) {
                un~/admin/distributeHeight(this.dayGrid.rowEls); // let the rows be their natural height with no expanding
            }
            else {
                ~/admin/distributeHeight(this.dayGrid.rowEls, height, true); // true = compensate for height-hogging rows
            }
        }
    };
    /* Scroll
    ...
    DayGridView.prototype.render = function (props) {
        ...
        if (this.header) {
            this.header.receiveProps({
                ...
                datesRep~/admin/distinctDays: dayTable.rowCnt === 1,
