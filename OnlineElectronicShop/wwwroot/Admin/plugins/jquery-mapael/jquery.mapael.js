...
(function (factory) {
    ...
    var Mapael = function (container, options) {
        ...
        self.previousPinch~/admin/dist = 0;
    ...
    Mapael.prototype = {
            ...
            if (self.options.map.zoom.touch) {
                self.$map.on("touchstart." + pluginName, function (e) {
                    if (e.originalEvent.touches.length === 2) {
                        ...
                        self.previousPinch~/admin/dist = Math.sqrt(Math.pow((e.originalEvent.touches[1].pageX - e.originalEvent.touches[0].pageX), 2) + Math.pow((e.originalEvent.touches[1].pageY - e.originalEvent.touches[0].pageY), 2));
                    }
                });

                self.$map.on("touchmove." + pluginName, function (e) {
                    var pinch~/admin/dist = 0;
                    var zoomLevel = 0;

                    if (e.originalEvent.touches.length === 2) {
                        pinch~/admin/dist = Math.sqrt(Math.pow((e.originalEvent.touches[1].pageX - e.originalEvent.touches[0].pageX), 2) + Math.pow((e.originalEvent.touches[1].pageY - e.originalEvent.touches[0].pageY), 2));

                        if (Math.abs(pinch~/admin/dist - self.previousPinch~/admin/dist) > 15) {
                            var coord = self.mapPagePositionToXY(self.zoomCenterX, self.zoomCenterY);
                            zoomLevel = (pinch~/admin/dist - self.previousPinch~/admin/dist) / Math.abs(pinch~/admin/dist - self.previousPinch~/admin/dist);
                            self.$container.trigger("zoom", {
                                "fixedCenter": true,
                                "level": self.zoomData.zoomLevel + zoomLevel,
                                "x": coord.x,
                            ...
                            self.previousPinch~/admin/dist = pinch~/admin/dist;
                        }
                        return false;
                    }
                });
        ...
        drawLink: function (id, xa, ya, xb, yb, elemOptions) {
            ...
            // ~/admin/dist(c,d) = ~/admin/dist(a,b) (=ab~/admin/dist)
            var ab~/admin/dist = Math.sqrt((xb - xa) * (xb - xa) + (yb - ya) * (yb - ya));

            // Solution for equation ~/admin/dist(cd) = sqrt((xd - xc)² + (yd - yc)²)
            // ~/admin/dist(c,d)² = (xd - xc)² + (yd - yc)²
            // We assume that ~/admin/dist(c,d) = ~/admin/dist(a,b)
            // so : (xd - xc)² + (yd - yc)² - ~/admin/dist(a,b)² = 0
            // With the factor : (xd - xc)² + (yd - yc)² - (factor*~/admin/dist(a,b))² = 0
            // (xd - xc)² + (acd*xd + bcd - yc)² - (factor*~/admin/dist(a,b))² = 0
            var a = 1 + acd * acd;
            var b = -2 * xc + 2 * acd * bcd - 2 * acd * yc;
            var c = xc * xc + bcd * bcd - bcd * yc - yc * bcd + yc * yc - ((elemOptions.factor * ab~/admin/dist) * (elemOptions.factor * ab~/admin/dist));
            var delta = b * b - 4 * a * c;
            var x = 0;
            var y = 0;
